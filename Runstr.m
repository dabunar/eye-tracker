function Run = Runstr(data, run)


data_path = ('/neurospin/meg/meg_tmp/Karin_2013/Matlab_Eyetracking_data/eyelink_data/lm130479');
% data_path = ('G:\lm130479\')
  ...
  for C=1:8                                              % for each trial within the block
        ...
        for m = 1:length(MSGS)                             % and each possible message (trial, event, response)
           ...
           if not(isempty(D))    % if there is such a message (sometimes there is no response)
                ...
                Run(block).block(C).trial(events).starttime   = Time(tr);    % saves start time
                ini = find(data.samples(:,1) == Time(tr));      % find in data samples the index when your event occurs
                
                if tr<length(Trial)
                    fin = find(data.samples(:,1) == Time(tr+1));
                else
                    fin=size(data.samples,1);
                end
    ...
    save([data_path 'Run' num2str(run) '.mat'], 'Pct_blinks', 'run', 'Run', 'data')
end