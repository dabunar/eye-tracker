 ...
 function matrix = ThecellMatrix(Run)
...
matrix = cell(264,7) ;  % 8*33 events, 7 colomns for infos
...
for block_num = 1:33; 
    ...
    matrix{count:count+7,3} = oRun(block_num).Ref(1);  % count 1 bis 1+7, da mach mir die Ref reind ie gerade dran ist
    ...
    for DimQuestion = 1:8 % loops across questions/ dimensions Space Time
        ...
        % Colonne pour les Dimensions
            count2=count2+1;
            if strcmpi(char(Run(block_num).block(DimQuestion).Question),'SPACE')   % 1 = SPace and 2 = Time
                matrix{count2,4} = 1;
            else
                matrix{count2,4} = 2;
            end
      ...
      % Colonne pour Distance effect (1=close, 2=far)
         count3=count3+1;
         if strcmpi(char(Run(block_num).block(DimQuestion).trial(2).type),'Close')  
                matrix{count3,6} = 1;
            else
                matrix{count3,6} = 2;
         end
